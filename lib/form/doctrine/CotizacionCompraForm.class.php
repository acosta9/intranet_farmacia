 ...
 */
class CotizacionCompraForm extends BaseCotizacionCompraForm
{
  public function configure()
  {
    ...
    $detgals = $this->getObject()->getCotizacionCompraDet();
    if (!$detgals){
      $detgal = new getCotizacionCompraDet();
      $detgal->setCotizacionCompra($this->getObject());
    ...
    foreach ($detgals as $detgal) {
      $detgals_form = new CotizacionCompraDetForm($detgal);
    ...
    $this->embedForm('cotizacion_compra_det', $detgals_forms);
  ...
  public function validaciones($validator, $values) {
    if($this->isNew()) {
      ...
      $count_ccs = Doctrine_Core::getTable('CotizacionCompra')
        ->createQuery('a')
  ...
  public function addDetalles($num){
    $detgal = new CotizacionCompraDet();
    $detgal->setCotizacionCompra($this->getObject());
    $detgal_form = new CotizacionCompraDetForm($detgal);

    //Empotramos la nueva pícture en el contenedor
    $this->embeddedForms['cotizacion_compra_det']->embedForm($num, $detgal_form);
    //Volvemos a empotrar el contenedor
    $this->embedForm('cotizacion_compra_det', $this->embeddedForms['cotizacion_compra_det']);
  }
  public function bind(array $taintedValues = null, array $taintedFiles = null)   {
    if(@$taintedValues['cotizacion_compra_det']) {
      foreach($taintedValues['cotizacion_compra_det'] as $key=>$newTodo) {
        if (!isset($this['cotizacion_compra_det'][$key])) {
