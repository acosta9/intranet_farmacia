 ...
 */
class InvEntradaForm extends BaseInvEntradaForm
{
  public function configure()
  {
    ...
    $detgals = $this->getObject()->getInvEntradaDet();
    if (!$detgals){
      $detgal = new getInvEntradaDet();
      $detgal->setInvEntrada($this->getObject());
    ...
    foreach ($detgals as $detgal) {
      $detgals_form = new InvEntradaDetForm($detgal);
    ...
    $this->embedForm('inv_entrada_det', $detgals_forms);
    ...
    $this->validatorSchema->setPostValidator(
      new sfValidatorAnd(array(
        new sfValidatorDoctrineUnique(array('model' => 'InvEntrada', 'column' => array('id')), array(
  ...
  public function validaciones($validator, $values) {
    if($this->isNew()) {
      ...
      $count_ccs = Doctrine_Core::getTable('InvEntrada')
        ->createQuery('a')
  ...
  public function addDetalles($num){
    $detgal = new InvEntradaDet();
    $detgal->setInvEntrada($this->getObject());
    $detgal_form = new InvEntradaDetForm($detgal);

    //Empotramos la nueva pícture en el contenedor
    $this->embeddedForms['inv_entrada_det']->embedForm($num, $detgal_form);
    //Volvemos a empotrar el contenedor
    $this->embedForm('inv_entrada_det', $this->embeddedForms['inv_entrada_det']);
  }
  public function bind(array $taintedValues = null, array $taintedFiles = null)   {
    if(@$taintedValues['inv_entrada_det']) {
      foreach($taintedValues['inv_entrada_det'] as $key=>$newTodo) {
        if (!isset($this['inv_entrada_det'][$key])) {
