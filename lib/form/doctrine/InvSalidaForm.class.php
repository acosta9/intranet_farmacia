 ...
 */
class InvSalidaForm extends BaseInvSalidaForm
{
  public function configure()
  {
    ...
    $this->widgetSchema['total']->setAttributes(array('class' => 'form-control number'));
    ...
    $detgals = $this->getObject()->getInvSalidaDet();
    if (!$detgals){
      $detgal = new getInvSalidaDet();
      $detgal->setInvSalida($this->getObject());
    ...
    foreach ($detgals as $detgal) {
      $detgals_form = new InvSalidaDetForm($detgal);
    ...
    $this->embedForm('inv_salida_det', $detgals_forms);
    ...
    $this->validatorSchema->setPostValidator(
      new sfValidatorAnd(array(
        new sfValidatorDoctrineUnique(array('model' => 'InvSalida', 'column' => array('id')), array(
  ...
  public function validaciones($validator, $values) {
    if($this->isNew()) {
      ...
      $count_ccs = Doctrine_Core::getTable('InvSalida')
        ->createQuery('a')
  ...
  public function addDetalles($num){
    $detgal = new InvSalidaDet();
    $detgal->setInvSalida($this->getObject());
    $detgal_form = new InvSalidaDetForm($detgal);

    //Empotramos la nueva pícture en el contenedor
    $this->embeddedForms['inv_salida_det']->embedForm($num, $detgal_form);
    //Volvemos a empotrar el contenedor
    $this->embedForm('inv_salida_det', $this->embeddedForms['inv_salida_det']);
  }
  public function bind(array $taintedValues = null, array $taintedFiles = null)   {
    if(@$taintedValues['inv_salida_det']) {
      foreach($taintedValues['inv_salida_det'] as $key=>$newTodo) {
        if (!isset($this['inv_salida_det'][$key])) {
