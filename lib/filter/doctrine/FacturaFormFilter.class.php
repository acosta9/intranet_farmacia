...
class FacturaFormFilter extends BaseFacturaFormFilter
{
  public function configure()
  {
    ...
    $this->setWidget('num_fact_fiscal', new sfWidgetFormFilterInput(array('with_empty' => false)));
    $this->setWidget('ndespacho', new sfWidgetFormFilterInput(array('with_empty' => false)));
    ...
    $this->widgetSchema['num_fact_fiscal']->setAttributes(array('class' => 'form-control'));
    ...
    $this->validatorSchema ['num_fact_fiscal'] = new sfValidatorPass();
    $this->validatorSchema ['ndespacho'] = new sfValidatorPass();
    $this->validatorSchema ['num_factura'] = new sfValidatorPass();
  }
  public function getFields() {
    ...
    $fields['cod'] = 'Text';
    $fields['num_fact_fiscal'] = 'Text';
    $fields['ndespacho'] = 'Text';
    $fields['num_factura'] = 'Text';
    return $fields;
  }
  //$result["num_fact_fiscal"].$result["ndespacho"].$result["num_factura"]
  public function addNumFactFiscalColumnQuery($query, $field, $value) {
    $valor=implode($value);
    if(strlen($valor)>1) {
      return $query->andwhere("num_fact_fiscal LIKE '$valor'");
    }
  }
  public function addNdespachoColumnQuery($query, $field, $value) {
    $valor=implode($value);
    if(strlen($valor)>1) {
      return $query->andwhere("ndespacho LIKE '$valor'");
    }
  }
  public function addNumFacturaColumnQuery($query, $field, $value) {
    $valor=implode($value);
    if(strlen($valor)>1) {
      return $query->andwhere("num_factura LIKE '$valor'");
    }
  }
  public function addCodColumnQuery($query, $field, $value) {
    $valor=implode($value);
    if(strlen($valor)>1) {
      return $query->andwhere("id = ?", $valor);
    }
  }
