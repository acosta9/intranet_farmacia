...
class sfWidgetFormJQueryAutocompleter extends sfWidgetFormInput
{
  ...
  public function render($name, $value = null, $attributes = array(), $errors = array())
  {
    ...
    $vowels = array("[", "]", "_");
    $accion = str_replace($vowels, "", $name)."()";

    return $this->renderTag('input', array('type' => 'hidden', 'name' => $name, 'value' => $value, 'onchange' => $this->getOption('onchange'))).
...
<script type="text/javascript">
  jQuery(document).ready(function() {
    ...
    .result(function(event, data) { jQuery("#%s").val(data[1]); document.getElementById("%s").onchange(); });
  });
</script>
EOF
      ,
