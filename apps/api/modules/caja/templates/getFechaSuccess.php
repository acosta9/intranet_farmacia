...
// caja_user //
  if($tipo=="createdat") {
    ...
    $results2 = $q->execute("SELECT updated_at as fecha FROM caja_user WHERE substring(caja_id,1,2)=$eid AND updated_at<>created_at ORDER BY updated_at DESC LIMIT 1");
  }  
  ...
  if($tipo=="createdat") {
    ...
    $results3 = $q->execute("SELECT updated_at as fecha FROM empresa_user WHERE empresa_id=$eid AND updated_at<>created_at ORDER BY updated_at DESC LIMIT 1");
  }
