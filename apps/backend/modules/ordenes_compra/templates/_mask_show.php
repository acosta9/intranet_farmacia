...
<?php $cotizacion_compra=Doctrine_Core::getTable('CotizacionCompra')->findOneBy('id',$form->getObject()->get('cotizacion_compra_id'));?>
</div></div></div>
    ...
    <div class="row invoice-info">
      <div class="col-sm-4 invoice-col">
        <address>
          ...
          <b>Email:</b> <?php echo $empresa->getEmail()?></b>
        </address>
      </div>
      <div class="col-sm-4 invoice-col">
        <address>
      ...
      <div class="col-sm-4 invoice-col">
        <b class="float-right">OC: #<?php echo ($form->getObject()->get('ncontrol')); ?></b><br>
        <small class="float-right">Emision: <?php echo(date("d/m/Y", strtotime($form->getObject()->get('created_at')))); ?></small><br/>
        <small class="float-right">Dias de Credito: <?php echo $ordenes_compra->getDiasCredito(); ?></small><br/>
        <small class="float-right"><b>Cotizacion: #<?php echo $cotizacion_compra->getNcontrol(); ?></b></small>
      </div>
    </div>
    <div class="row">
        ...
        <table class="table table-striped">
          ...
          <tbody>
            ...
            $results = Doctrine_Query::create()
              ...
              ->from('OrdenesCompraDet ccd')
              ->leftJoin('ccd.Producto p')
              ->where('ccd.ordenes_compra_id = ?', $form->getObject()->get('id'))
              ->orderBy('ccd.id ASC')
              ->execute();
            $total=0;
  ...
  <div class="row no-print">
    <div class="col-12">
      ...
      <?php if($form->getObject()->get('estatus')==3): ?>
        <a href="<?php echo url_for("@ordenes_compra")."/cerrar?id=".$form->getObject()->get('id')?>" class="btn btn-success" >
          Cerrar OC
        </a>
      <?php endif; ?>
      <?php if($form->getObject()->get('estatus')!=5): ?>
        <?php if($form->getObject()->get('estatus')==2 || $form->getObject()->get('estatus')==3): ?>
          <a href="<?php echo url_for("@factura_compra")."/new?oc=1&id=".$form->getObject()->get('id')?>" class="btn btn-warning" >
            <i class="fas fa-file-invoice"></i> Convertir a FC
          </a>
        <?php endif; ?>
...
<script>
  function printDiv(divName) {
    document.title='ordenes_compra_<?php echo $form->getObject()->get('ncontrol'); ?>.pdf';
    $("#"+divName).printThis({
      pageTitle: 'ordenes_compra_<?php echo $form->getObject()->get('ncontrol'); ?>.pdf',
    });
  }
  function anular() {
    var retVal = confirm("¿Estas seguro de anular la cotizacion de compra?");
    if( retVal == true ){
        location.href = "<?php echo url_for("ordenes_compra")."/anular?id=".$form->getObject()->get('id')?>";
    }else{
     return false;
    }
  }
