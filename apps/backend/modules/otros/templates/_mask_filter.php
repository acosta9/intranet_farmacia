...
<div class="col-md-6">
  <div class="form-group">
    <label for="otros_filters_nombre">Tipo Tasa</label>
...
<script type="text/javascript">
  $( document ).ready(function() {
    $("#otros_filters_empresa_id").select2({ width: '100%' });
    $("#otros_filters_creado_por").select2({ width: '100%' });
  });
</script>
...
<script type="text/javascript">
  $( document ).ready(function() {
    var j=0;
    $("#otros_filters_empresa_id option").each(function() {
      var id_old=$(this).val();
      var i=0;
      ...
      if(i==0) {
        if($("#otros_filters_empresa_id option[value='"+id_old+"']").is(':selected')) {
          j++;
        }
        $("#otros_filters_empresa_id option[value='"+id_old+"']").remove();
      }
    });
    if(j>0) {
      $('#otros_filters_empresa_id option').prop('selected', true);
      $( "#form_filter" ).submit();
    }
    if ($("#otros_filters_empresa_id").find('option:selected').length== 0) {
      $('#otros_filters_empresa_id option').prop('selected', true);
      $( "#form_filter" ).submit();
    }
  });
