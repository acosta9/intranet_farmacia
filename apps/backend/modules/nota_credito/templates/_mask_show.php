  ...
  <div class="invoice p-3 mb-3" id="invoice" <?php if($form->getObject()->get('estatus')==3) { echo 'style="background: #f1daa759 !important;"'; }?>>
    ...
    <div class="row invoice-info">
      ...
      <div class="col-sm-4 invoice-col">
        ...
        <b class="float-right">NOTA DE CREDITO PAGO</b><br/>
    ...
    <div class="row" style="margin-top: 2rem">
      ...
      <div class="col-md-6" style="margin-bottom: 0.7rem;">
        <b>BAJO LA FORMA DE PAGO: </b><span style="text-decoration: underline;"><?php echo $nota_credito->getFormaPago()." (".$nota_credito->getCoin().")"; ?></span>
      </div>
      <?php if($nota_credito->getMoneda()==1) { ?>
        <div class="col-md-3" style="margin-bottom: 0.7rem;">
          <b>CANTIDAD EN BS: </b><span style="text-decoration: underline;" class="number"><?php echo number_format(number_float($nota_credito->getMonto())*number_float($nota_credito->getTasaCambio()), 4, '.', ','); ?></span>
        </div>
      <?php } ?>
      <div class="col-md-3" style="margin-bottom: 0.7rem;">
        <b>CANTIDAD EN USD: </b><span style="text-decoration: underline;" class="number"><?php echo number_format(number_float($nota_credito->getMonto()), 4, '.', ','); ?></span>
      </div>
      ...
      <div class="col-md-6" style="margin-bottom: 0.7rem;">
        <b>RECIBIDO POR: </b> <span style="text-decoration: underline;"><?php echo $nota_credito->getCreator() ?></span>
      </div>
      <div class="col-md-6" style="margin-bottom: 0.7rem;">
      ...
      <div class="row">
        <div class="col-12 table-responsive">
          <table class="table table-striped">
            ...
            <tbody>
              <tr>
                <td style="vertical-align: middle"><?php echo "USD ".number_format($nota_credito->getMonto(), 4, '.', ','); ?></td>
                <td style="vertical-align: middle"><?php echo "USD ".number_format($nota_credito->getMontoFaltante(), 4, '.', ','); ?></td>
                <td>
                  <?php
                    ...
                    } else if($nota_credito->getEstatus()==3) {
            ...
            <tbody>
              <?php
                $results = Doctrine_Query::create()
                  ...
                  ne.id as neid, ne.ncontrol as necontrol')
                  ->from('NotaCreditoDet ncd')
                  ->leftJoin('ncd.CuentasCobrar cc')
              ...
              <tr>
                <td style="vertical-align: middle">
                  <?php if($result["ccfid"]) {
                    echo "FACTURA: <a target='_blank' href='".url_for("factura")."/show?id=".$result["ccfid"]."'>".$result["numfact"]."</a>";
                  } else {
                    echo "NOTA EN: <a target='_blank' href='".url_for("nota_entrega")."/show?id=".$result["ccneid"]."'>".$result["necontrol"]."</a>";
                  } ?>
                </td>
               ...
               <td>
                 <?php
                 if($result["ncdanulado"]==1) { ?>
                   <a href="#" onclick="anular_item(<?php echo $nota_credito->getId().",".$result->getId(); ?>)" class="btn btn-danger float-right disabled">
                     <i class="fas fa-minus-circle"></i> Anular
                   </a>
                 <?php } else { ?>
                   <a href="#" onclick="anular_item(<?php echo $nota_credito->getId().",".$result->getId(); ?>)" class="btn btn-danger float-right">
                     <i class="fas fa-minus-circle"></i> Anular
                   </a>
                 <?php } ?>
               </td>
...
<script>
  ...
  function anular() {
    ...
    if( retVal == true ){
        location.href = "<?php echo url_for("nota_credito")."/anular?id=".$form->getObject()->get('id')?>";
    }else{
     return false;
    }
  }
  function anular_item(ncid, id) {
    var retVal = confirm("¿Estas seguro de anular el detalle de la nota de credito?");
    if( retVal == true ){
        location.href = "<?php echo url_for("nota_credito")."/anularItem?ncid="?>"+ncid+"&id="+id;
    }else{
     return false;
    }
  }
