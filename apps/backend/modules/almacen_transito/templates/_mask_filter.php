...
<script type="text/javascript">
  $( document ).ready(function() {
    $("#almacen_transito_filters_empresa_id").select2({ width: '100%' });
    $("#almacen_transito_filters_estatus").select2({ width: '100%' });
  });
</script>
...
<script type="text/javascript">
  $( document ).ready(function() {
    var j=0;
    $("#almacen_transito_filters_empresa_id option").each(function() {
      var id_old=$(this).val();
      var i=0;
      ...
      if(i==0) {
        if($("#almacen_transito_filters_empresa_id option[value='"+id_old+"']").is(':selected')) {
          j++;
        }
        $("#almacen_transito_filters_empresa_id option[value='"+id_old+"']").remove();
      }
    });
    if(j>0) {
      $('#almacen_transito_filters_empresa_id option').prop('selected', true);
      $( "#form_filter" ).submit();
    }
    if ($("#almacen_transito_filters_empresa_id").find('option:selected').length== 0) {
      $('#almacen_transito_filters_empresa_id option').prop('selected', true);
      $( "#form_filter" ).submit();
    }
  });
