    ...
    <div class="row" style="margin-top: 2rem">
      ...
      <?php if($recibo_pago->getMoneda()==1) { ?>
        <div class="col-md-3" style="margin-bottom: 0.7rem;">
          <b>CANTIDAD EN BS: </b><span style="text-decoration: underline;" class="number"><?php echo number_format(number_float($recibo_pago->getMonto())*number_float($recibo_pago->getTasaCambio()), 4, '.', ','); ?></span>
        </div>
      <?php } ?>
      <div class="col-md-3" style="margin-bottom: 0.7rem;">
        <b>CANTIDAD EN USD: </b><span style="text-decoration: underline;" class="number"><?php echo number_format(number_float($recibo_pago->getMonto()), 4, '.', ','); ?></span>
      </div>
...
<script>
  ...
  function anular() {
    ...
    if( retVal == true ){
        location.href = "<?php echo url_for("recibo_pago")."/anular?id=".$form->getObject()->get('id')?>";
    }else{
     return false;
    }
  }
