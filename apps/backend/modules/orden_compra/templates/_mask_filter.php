...
<script type="text/javascript">
  $( document ).ready(function() {
    $("#orden_compra_filters_empresa_id").select2({ width: '100%' });
    $("#orden_compra_filters_creado_por").select2({ width: '100%' });
    $("#orden_compra_filters_updated_por").select2({ width: '100%' });
    $("#orden_compra_filters_estatus").select2({ width: '100%'});
  });
</script>
...
<script type="text/javascript">
  $( document ).ready(function() {
    var j=0;
    $("#orden_compra_filters_empresa_id option").each(function() {
      var id_old=$(this).val();
      var i=0;
      ...
      if(i==0) {
        if($("#orden_compra_filters_empresa_id option[value='"+id_old+"']").is(':selected')) {
          j++;
        }
        $("#orden_compra_filters_empresa_id option[value='"+id_old+"']").remove();
      }
    });
    if(j>0) {
      $('#orden_compra_filters_empresa_id option').prop('selected', true);
      $( "#form_filter" ).submit();
    }
    if ($("#orden_compra_filters_empresa_id").find('option:selected').length== 0) {
      $('#orden_compra_filters_empresa_id option').prop('selected', true);
      $( "#form_filter" ).submit();
    }
  });
