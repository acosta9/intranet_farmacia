<?php
  $value = array();
  $i=0;
  foreach ($inv_entradas as $inv_entrada):
    $value[$i]=$inv_entrada["id"];
    $i++;
  endforeach;
?>
  ...
  <table style="border-spacing: 0px; width: 100%;" id="tabla_export">
    <thead>
      <tr>
        <td colspan="11" style="text-align: center; text-decoration: underline;">
          <p style="margin-top: 0px !important">
            <b>REPORTE DE ENTRADA DE INVENTARIO</b>
      ...
      <tr style="">
        ...
        <td class="bbottom tleft"><b>ENTRADA</b></td>
    ...
    <tbody>
      <?php
        if (count($value)>0):
          ...
          id.nombre as idname,
          ied.qty as qty, ied.fecha_venc as fvenc, ied.lote as lot, 
          ie.id as ieid, ie.created_at as fentrada, ie.anulado as anulado,
          p.nombre as pname, p.serial as pserial, 
          pu.nombre as puname
          FROM inv_entrada_det ied
      ...
      ?>
          <tr style="<?php echo $background;?>">
            ...
            <td class="bbottom tleft"><?php echo date("d/m/Y H:i:s", strtotime($result["fentrada"])); ?></td>
            <td class="bbottom tcenter"><?php echo $result["lot"]; ?></td>
            <td class="bbottom tleft"><?php echo date("d/m/Y", strtotime($result["fvenc"])); ?></td>
...
<script>
  function toPdf() {
    ...
    document.title='entradas_de_inventario_'+fecha+'.pdf';
    style.type = 'text/css';
    style.media = 'print';
  ...
  function toExcel() {
    ...
    $("#tabla_export").table2excel({
      filename: 'entradas_de_inventario_'+fecha+'.xls',
      preserveColors: true 
    });
  }
</script>
