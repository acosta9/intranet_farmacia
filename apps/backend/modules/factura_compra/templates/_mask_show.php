...
<?php $ordenes_compra=Doctrine_Core::getTable('OrdenesCompra')->findOneBy('id',$form->getObject()->get('ordenes_compra_id'));?>
</div></div></div>
  <div class="invoice p-3 mb-3" id="invoice" <?php if($form->getObject()->get('estatus')==4) { echo 'style="background: #f1daa759 !important;"'; }?>>
    ...
    <div class="row invoice-info">
      ...
      <div class="col-sm-4 invoice-col">
        ...
        <small class="float-right">Dias de Credito: <?php echo $factura_compra->getDiasCredito(); ?></small><br/>
        <small class="float-right"><b>OC: #<?php echo $ordenes_compra->getNcontrol(); ?></b></small>
      </div>
    </div>
    <div class="row">
      <div class="col-12 table-responsive">
        <table class="table table-striped">
          <tbody>
            <tr>
              <?php $deposito=Doctrine_Core::getTable('InvDeposito')->findOneBy('id',$form->getObject()->get('deposito_id'));?>
              <?php $emp_dep=Doctrine_Core::getTable('Empresa')->findOneBy('id',$deposito->getEmpresaId());?>
              <td style="text-align: center;"><b>DEPOSITO:</b> <?php echo $emp_dep->getNombre(); ?><br><?php echo $deposito->getNombre(); ?></td>
              <td style="text-align: center;"><b>INGRESO A INVENTARIO:</b> <?php if($form->getObject()->get('sumar_inv')==1) { echo "SUMAR"; } else { echo "NO SUMAR"; } ?></td>
            </tr>
          </tbody>
        </table>
      </div>
        ...
        <table class="table table-striped">
          <thead>
            <tr>
              ...
              <th>LOTE y VENC</th>
              <th style="text-align: right">COSTO PREVIO</th>
              <th style="text-align: right">UTIL(%) NUEVA</th>
          ...
          <tbody>
            ...
            $results = Doctrine_Query::create()
              ->select("FORMAT(REPLACE(fd.qty, ' ', ''), 0, 'de_DE') as qty, fd.exento as exento,
              fd.price_unit as punit, fd.price_calculado as pcalculado, fd.price_unit_old as punitold, 
              fd.price_tot as ptot, i.id as iid, fd.fecha_venc as fvenc, fd.lote as lote,
              p.nombre as nombre, p.serial as serial, p.util_usd_8 as util8")
              ->from('FacturaCompraDet fd, fd.Inventario i, i.Producto p')
              ->where('fd.factura_compra_id = ?', $form->getObject()->get('id'))
              ->orderBy('fd.id ASC')
              ->execute();
            $total=0;
            ...
            <tr>
              ...
              <td style="vertical-align: middle">
                <?php
                  $phpdate = strtotime($result["fvenc"]);
                  echo "<b>".$result["lote"]."</b> <small>".date('M-Y', $phpdate)."</small><br/>";
                ?>
              </td>
              <td style="vertical-align: middle; text-align: right"><?php echo "USD <span class='moneyStr'>".$result["punitold"];?></span></td>
              <td style="vertical-align: middle; text-align: right"><?php echo "<span class='moneyStr'>".$result["util8"];?></span>%</td>
              <td style="vertical-align: middle; text-align: right"><?php echo "USD <span class='moneyStr'>".$result["pcalculado"];?></span></td>
      ...
      <div class="row">
        <div class="col-12 table-responsive">
          <table class="table table-striped">
            ...
            <tbody>
              <?php
                $cuentas_pagar = Doctrine_Core::getTable('CuentasPagar')->findOneBy('factura_compra_id', $form->getObject()->get('id'));
              ?>
              <tr>
...
<script>
  function printDiv(divName) {
    document.title='factura_compra_<?php echo $form->getObject()->get('ncontrol'); ?>.pdf';
    $("#"+divName).printThis({
      pageTitle: 'factura_compra_<?php echo $form->getObject()->get('ncontrol'); ?>.pdf',
    });
  }
  function anular() {
