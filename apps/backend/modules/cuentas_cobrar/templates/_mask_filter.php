...
<script type="text/javascript">
  $( document ).ready(function() {
    $("#cuentas_cobrar_filters_empresa_id").select2({ width: '100%' });
    $("#cuentas_cobrar_filters_estatus").select2({ width: '100%' });
  });
</script>
...
<script type="text/javascript">
  $( document ).ready(function() {
    var j=0;
    $("#cuentas_cobrar_filters_empresa_id option").each(function() {
      var id_old=$(this).val();
      var i=0;
      ...
      if(i==0) {
        if($("#cuentas_cobrar_filters_empresa_id option[value='"+id_old+"']").is(':selected')) {
          j++;
        }
        $("#cuentas_cobrar_filters_empresa_id option[value='"+id_old+"']").remove();
      }
    });
    if(j>0) {
      $('#cuentas_cobrar_filters_empresa_id option').prop('selected', true);
      $( "#form_filter" ).submit();
    }
    if ($("#cuentas_cobrar_filters_empresa_id").find('option:selected').length== 0) {
      $('#cuentas_cobrar_filters_empresa_id option').prop('selected', true);
      $( "#form_filter" ).submit();
    }
  });
