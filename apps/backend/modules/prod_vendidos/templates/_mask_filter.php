...
<div class="col-md-6">
  <div class="form-group">
    <label for="prod_vendidos_filters_cod">Usuario</label>
...
<div class="col-md-4">
  <div class="form-group">
    <label for="prod_vendidos_filters_anulado">Oferta</label>
    <?php echo $form['oferta']->render(array('class' => 'form-control'))?>
    <?php if ($form['oferta']->renderError())  { echo $form['oferta']->renderError(); } ?>
  </div>
</div>
<div class="col-md-8">
  <div class="form-group">
...
<script type="text/javascript">
  $( document ).ready(function() {
    ...
    $("#prod_vendidos_filters_user_id").select2({ width: '100%' });
    ...
    $("#prod_vendidos_filters_producto").select2({
      ...
      ajax: {
        url: '<?php echo url_for("prod_vendidos")."/getProductos"; ?>',
        dataType: 'json',
    ...
    $("#prod_vendidos_filters_laboratorio_id").select2({
      ...
      ajax: {
        url: '<?php echo url_for("prod_vendidos")."/getLaboratorios"; ?>',
        dataType: 'json',
        headers : {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        delay: 250,
        type: 'GET',
    ...
    $('#prod_vendidos_filters_deposito_id').load('<?php echo url_for('prod_vendidos/depositoFilters') ?>?id='+emps.join(",")+'&did='+deposito_id).fadeIn("slow");
  });
  ...
  $(function () {
    $("#prod_vendidos_filters_empresa_id").on('change', function(event){
      ...
      $('#prod_vendidos_filters_deposito_id').load('<?php echo url_for('prod_vendidos/depositoFilters') ?>?id='+emps.join(",")).fadeIn("slow");
    });
  });
</script>