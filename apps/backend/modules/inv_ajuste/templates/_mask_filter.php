...
<script type="text/javascript">
  $( document ).ready(function() {
    var j=0;
    $("#inv_ajuste_filters_empresa_id option").each(function() {
      var id_old=$(this).val();
      var i=0;
      ...
      if(i==0) {
        if($("#inv_ajuste_filters_empresa_id option[value='"+id_old+"']").is(':selected')) {
          j++;
        }
        $("#inv_ajuste_filters_empresa_id option[value='"+id_old+"']").remove();
      }
    });
    if(j>0) {
      $('#inv_ajuste_filters_empresa_id option').prop('selected', true);
      $( "#form_filter" ).submit();
    }
    if ($("#inv_ajuste_filters_empresa_id").find('option:selected').length== 0) {
      $('#inv_ajuste_filters_empresa_id option').prop('selected', true);
      $( "#form_filter" ).submit();
    }
    $("#inv_ajuste_filters_empresa_id").select2({ width: '100%' });
    $("#inv_ajuste_filters_deposito_id").select2({ width: '100%' });
    $("#inv_ajuste_filters_creado_por").select2({ width: '100%' });
    $("#inv_ajuste_filters_updated_por").select2({ width: '100%' });
    $("#inv_ajuste_filters_unidad_id").select2({ width: '100%' });
    $("#inv_ajuste_filters_categoria_id").select2({ width: '100%' });

    var emps = [];
    $.each($("#inv_ajuste_filters_empresa_id option:selected"), function(){
      emps.push($(this).val());
    });
    var deposito_id=$('#inv_ajuste_filters_deposito_id').val();
    $('#inv_ajuste_filters_deposito_id').load('<?php echo url_for('inv_ajuste/depositoFilters') ?>?id='+emps.join(",")+'&did='+deposito_id).fadeIn("slow");
    ...
    if(prods.length>2) {
      ...
      for (index = 0; index < res.length; index++) {
        if(res[index].length>1) {
          ...
          $("#inv_ajuste_filters_producto").append("<option value='"+res2[0]+"' selected>"+res2[1]+"</option>");
        }
      }
    }
    ...
    if(comps.length>2) {
      ...
      for (index = 0; index < res.length; index++) {
        if(res[index].length>1) {
          var res2 = res[index].split("|");
          $("#inv_ajuste_filters_compuesto_id").append("<option value='"+res2[0]+"' selected>"+res2[1]+"</option>");
        }
      }
    }
    ...
    if(labs.length>2) {
      ...
      for (index = 0; index < res.length; index++) {
        if(res[index].length>1) {
          var res2 = res[index].split("|");
          $("#inv_ajuste_filters_laboratorio_id").append("<option value='"+res2[0]+"' selected>"+res2[1]+"</option>");
        }
      }
    }

    $("#inv_ajuste_filters_producto").select2({
      width: '100%',
      multiple: true,
      language: {
      ...
      ajax: {
        url: '<?php echo url_for("inv_ajuste")."/getProductos"; ?>',
        dataType: 'json',
    ...
    $("#inv_ajuste_filters_compuesto_id").select2({
      width: '100%',
      multiple: true,
      language: {
        inputTooShort: function () {
      ...
      ajax: {
        url: '<?php echo url_for("inv_ajuste")."/getCompuestos"; ?>',
        dataType: 'json',
        headers : {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        delay: 250,
        type: 'GET',
    ...
    $("#inv_ajuste_filters_laboratorio_id").select2({
      width: '100%',
      multiple: true,
      language: {
        inputTooShort: function () {
      ...
      ajax: {
        url: '<?php echo url_for("inv_ajuste")."/getLaboratorios"; ?>',
        dataType: 'json',
        headers : {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
        delay: 250,
        type: 'GET',
  ...
  $("#inv_ajuste_filters_empresa_id").on('change', function(event){
    var emps = [];
    $.each($("#inv_ajuste_filters_empresa_id option:selected"), function(){
      emps.push($(this).val());
    });
    $('#inv_ajuste_filters_deposito_id').hide();
    $('#inv_ajuste_filters_deposito_id').load('<?php echo url_for('inv_ajuste/depositoFilters') ?>?id='+emps.join(",")).fadeIn("slow");
  });
</script>