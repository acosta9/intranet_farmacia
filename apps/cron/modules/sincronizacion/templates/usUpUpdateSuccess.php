<?php
 ...
 $curl = curl_init();
  curl_setopt_array($curl, array(
    CURLOPT_URL => "http://$domain/api.php/usuario/getFecha?tipo=updatedat",
 ...
 if(!empty($data[0]['fecha3']))
  ...
  $usuarios=array();
  if(!empty($data[0]['fecha1'])){
    $results = $q->execute("SELECT * FROM sf_guard_user WHERE  updated_at>'$fechaus'");
  ...
  $permisos=array();
  if(!empty($data[0]['fecha2'])){
    $results2 = $q->execute("SELECT * FROM sf_guard_permission WHERE  updated_at>'$fechap'");
...
// sf_guard_user_permission //
 $upermisos=array();
 if(!empty($data[0]['fecha3'])){
    $results3 = $q->execute("SELECT * FROM sf_guard_user_permission WHERE updated_at>'$fechaup'");
 ...
 echo $data_update = json_encode($usData);
  ...
  $curl = curl_init();
  curl_setopt_array($curl, array(
    CURLOPT_URL => "http://$domain/api.php/usuario/post?tipo=updatedat",
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => '',
    CURLOPT_MAXREDIRS => 10,
    CURLOPT_TIMEOUT => 30,
