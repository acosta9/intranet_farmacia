<?php
 ...
 $curl = curl_init();
  curl_setopt_array($curl, array(
    CURLOPT_URL => "http://$domain/api.php/variosprod/getFecha?tipo=createdat",
...
if(!empty($data[0]['fecha3']))
  ...
  $produs=array();
  if(!empty($data[0]['fecha1'])){
    $results = $q->execute("SELECT * FROM prod_unidad WHERE created_at>'$fechapu'");
  ...
  $prodls=array();
  if(!empty($data[0]['fecha2'])){
    $results2 = $q->execute("SELECT * FROM prod_laboratorio WHERE created_at>'$fechapl'");
  ...
  $prodcs=array();
  if(!empty($data[0]['fecha3'])){
    $results3 = $q->execute("SELECT * FROM prod_categoria WHERE created_at>'$fechapc'");
 ...
 echo $data_update = json_encode($vpData);
  ...
  $curl = curl_init();
  curl_setopt_array($curl, array(
    CURLOPT_URL => "http://$domain/api.php/variosprod/post?tipo=createdat",
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => '',
    CURLOPT_MAXREDIRS => 10,
    CURLOPT_TIMEOUT => 30,
