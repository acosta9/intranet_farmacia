<?php
 ...
 $curl = curl_init();
  curl_setopt_array($curl, array(
    CURLOPT_URL => "http://$domain/api.php/traslado/getFecha2?eid=$eidh",
  ...
  // traslado //
  $traslado=array();
  $results = $q->execute("SELECT * FROM traslado WHERE empresa_desde=$eidd AND empresa_hasta=$eidh && updated_at>'$fecha'");
 ...
 echo $data_update = json_encode($traslado);
  ...
  $curl = curl_init();
  curl_setopt_array($curl, array(
    CURLOPT_URL => "http://$domain/api.php/traslado/postfarm",
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => '',
    CURLOPT_MAXREDIRS => 10,
    CURLOPT_TIMEOUT => 45,
