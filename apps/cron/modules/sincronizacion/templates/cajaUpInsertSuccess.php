...
// caja, caja_user, empresa_user
 ...
 $curl = curl_init();
  curl_setopt_array($curl, array(
    CURLOPT_URL => "http://$domain/api.php/caja/getFecha?eid=$eid&tipo=createdat",
 ...
 if(!empty($data[0]['fecha3']))
  ...
  $cajas=array();
  if(!empty($data[0]['fecha1'])){
    $results = $q->execute("SELECT * FROM caja WHERE empresa_id = '$eid' AND created_at>'$fechac'");
  ...
  $cajasu=array();
  if(!empty($data[0]['fecha2'])){
    $results2 = $q->execute("SELECT * FROM caja_user WHERE substring(caja_id,1,2)=$eid AND created_at>'$fechacu'");
  ...
  $empuss=array();
  if(!empty($data[0]['fecha3'])){
    $results3 = $q->execute("SELECT * FROM empresa_user WHERE empresa_id = '$eid' AND created_at>'$fechaeu'");
 ...
 echo $data_update = json_encode($caData);
  ...
  $curl = curl_init();
  curl_setopt_array($curl, array(
    CURLOPT_URL => "http://$domain/api.php/caja/post?tipo=createdat",
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => '',
    CURLOPT_MAXREDIRS => 10,
    CURLOPT_TIMEOUT => 30,
