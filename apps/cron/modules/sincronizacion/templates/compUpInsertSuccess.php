<?php
 ...
 $curl = curl_init();
  curl_setopt_array($curl, array(
    CURLOPT_URL => "http://$domain/api.php/compuesto/getFecha?tipo=createdat",
 ...
 if(!empty($data[0]['fecha2']))
  ...
  $compuestos=array();
  if(!empty($data[0]['fecha1'])){
    $results = $q->execute("SELECT * FROM compuesto WHERE created_at>='$fechac'");
  ...
  $prodcps=array();
  if(!empty($data[0]['fecha2'])){
    $results2 = $q->execute("SELECT prod_compuesto.producto_id, prod_compuesto.compuesto_id FROM producto INNER JOIN prod_compuesto ON producto.id = prod_compuesto.producto_id WHERE producto.created_at>='$fechap' ");
 ...
 echo $data_update = json_encode($compData);
  ...
  $curl = curl_init();
  curl_setopt_array($curl, array(
    CURLOPT_URL => "http://$domain/api.php/compuesto/post?tipo=createdat",
    CURLOPT_RETURNTRANSFER => true,
    CURLOPT_ENCODING => '',
    CURLOPT_MAXREDIRS => 10,
    CURLOPT_TIMEOUT => 30,
